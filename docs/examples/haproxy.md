...
We iterate over all directories under /config (the services) and create a **url_beg** and a **hdr_beg(host)** acl if the service has a scheme configured.
...
Optional template block to expose a service on a host port:
...
[Dec 16 18:26:20]  INFO remco[1]: Target config has been updated config=/etc/haproxy/haproxy.cfg resource=hapco source=resource.go:66
...
Now every container gets automatically registered under /services.
